...
void MotorControl_OnCANISR(void)
{
    ...
    // Heartbeat
    if (g_heartbeat_period_ms != 0xFFFF)
        {
            heartbeat_counter++;
            if (heartbeat_counter >= g_heartbeat_period_ms)
            {
                heartbeat_counter = 0;
    ...
    // Telemetria
    if (g_telemetry_period_ms != 0xFFFF)
    {
        telemetry_counter++;
        if (telemetry_counter >= g_telemetry_period_ms)
        {
            telemetry_counter = 0;
