...
plot(speed_actual.t, speed_actual.value, 'k', 'DisplayName', 'n (estymacja)');
plot(speed_ref.t, speed_ref.value, 'r--', 'DisplayName', 'n_{ref} (wartość zadana)');
...
xlabel('Czas t [s]');
ylabel('Prąd i_{d}, i_{q} [A]');
ylim([-0.2, 0.2]);
xlim([0,t_max]);
% legend({'i_d','i_q'}, 'Location', 'southoutside','Orientation','horizontal');
legend({'i_d','i_q'}, 'Location', 'southoutside','Orientation','horizontal');
